    ...
    <div class="row">
        ...
        <div>
            ...
            <div class="form-label-group position-relative">
                <input class="form-control" name="folderThumbnail" type="file" id="inputImage" accept="image/*"
                       onchange="setThumbnail(this);"/>
                <br>
            </div>

            <c:if test="${isModify eq true}">
            <div id="previewArea" style="display:
            <c:if test="${toModifyItem.thumbnail ne null && toModifyItem.thumbnail ne ''}">block</c:if>
                <c:if test="${toModifyItem.thumbnail eq null || toModifyItem.thumbnail eq ''}">none</c:if>;">
                <img id="preview"
                     <c:if test="${toModifyItem.thumbnail ne null && toModifyItem.thumbnail ne ''}">src="${pageContext.request.contextPath}/attachment/${toModifyItem.thumbnail}"</c:if>
            ...
            <div class="col-md-7 mb-3">
                <div class="form-label-group position-relative">
                    <input type="text" class="form-control" name="title" id="inputTitle" placeholder="제목"
                           <c:if test="${isModify eq true}">value="${toModifyItem.title}"</c:if> maxlength="100"
                           required>
        ...
        <div class="row">
            <div class="col-md-7 mb-3">
                <div class="form-floating">
                    <textarea class="form-control" name="memo" id="inputMemo" rows="3" maxlength="1000"><c:if
                            test="${isModify eq true}">${toModifyItem.memo}</c:if></textarea>
        ...
        <div class="row">
            <div class="col-md-5 mb-3">
                ...
                <div class="form-label">
                    <div class="border form-outline overflow-auto" style="height: 300px; padding: 10px">
                        ...
                        <input onkeyup="searchKeyword(this)" type="search" id="itemSearch" class="form-control"
                               placeholder="북마크 검색"/>
                        ...
                        <div id="toAddBookmarkArea">
                            <c:forEach items="${bookmarkList}" var="item">
                                <div class="card w-100" id="itemCard_${item.id}" style="margin-bottom: 10px">
                                    ...
                                    <h5 class="card-title" id="itemTitle_${item.id}"
                                        style="padding-left: 5px;">${item.title}</h5>
                                    <p class="card-text" id="itemMemo_${item.id}"
                                       style="padding-left: 5px;">${item.memo}</p>
                                    <p class="card-text" id="itemUrl_${item.id}"
                                       style="padding-left: 5px;">${item.url}</p>
                                    <div class="text-end" style="padding: 5px">
                                        <button type="button" onclick="addBookmark('${item.id}')"
            ...
            <div class="col-md-5 mb-3">
                ...
                <div class="form-label">
                    <div class="border form-outline overflow-auto" id="addedBookmarkArea"
        ...
        <div class="form-check">
            <input class="form-check-input" name="isShared" type="checkbox" value="true" id="shareCheckBox"
                   <c:if test="${isModify eq true}">
                   <c:if test="${toModifyItem.shared}">checked</c:if>
            </c:if>>
        ...
        <div class="form-check">
            <input class="form-check-input" name="isStared" type="checkbox" value="true" id="starCheckbox"
                   <c:if test="${isModify eq true}">
                   <c:if test="${toModifyItem.stared}">checked</c:if>
            </c:if>>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <a href="${pageContext.request.contextPath}/folder">
